...
//                sendGift(giftId);
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    Result<Wallet> result = LiveManager.getInstance().
                            givingGift(EMClient.getInstance().getCurrentUser(), anchorId, giftId, 1);
                    if(result!=null){
                        if(result.isRetMsg()){
                            isSuccess = true;
                            runOnUiThread(new Runnable() {
    ...
    public void onPresentImageClick(int giftId) {
        L.e(TAG, "onPresentImageClick()");
